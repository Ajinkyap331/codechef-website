import React, {useState} from 'react'
import './Team.css'
import { Avatar } from '@mui/material'
import { Navbar } from '../Components/Navbar'
import { Teams } from '../Data/Data'


export const Team = () => {
  var photo = "data:image/jpeg;base64,"

  const [selected, setselected] = useState("CP")

  return (
    <div>
      <Navbar />
      <div className="team-body">
        <div className="team-president">
          <p>President</p>
          <Avatar src={photo} sx={{ width: 250, height: 250 }} />
          <p>Rahul Badgujar</p>
        </div>
        <div className='team-teams'>
          <div>
            <section onClick={() => {setselected("CP")}} >CP TEAM</section>
            <section onClick={() => {setselected("DESIGN")}} >DESIGN TEAM</section>
            <section onClick={() => {setselected("EVENT")}} >EVENT TEAM</section>
            <section onClick={() => {setselected("SOCIAL MEDIA")}} >SOCIAL MEDIA TEAM</section>
          </div>
          <div>
            <section>{selected} TEAM</section>
          </div>
          <div  className = "team-card">
            <Avatar src={photo} sx={{ width: 200, height: 200 }} />
            <p>{Teams[selected].lead.name}</p>
          </div>
          <div>
            {
              Teams[selected].members.map((e) => {
                return (
                  <div className = "team-card">
                    <Avatar src={photo} sx={{ width: 150, height: 150 }} />
                    <p>{e.name}</p>
                  </div>
                )
              })
            }
          </div>
        </div>
      </div>
    </div>
  )
}

